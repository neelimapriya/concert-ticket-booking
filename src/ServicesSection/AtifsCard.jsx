...
const AtifsCard = ({ atif }) => {
  const { id, name, description, price, location, thumbnail, date } = atif;
  ...
  const handleAddBook=()=>{
    ...
    if(!bookedItem){
      ...
      const itemExist=bookedItem.find(atif =>atif.id == id)

      if(!itemExist){
        addedBooked.push(...bookedItem, atif)
        localStorage.setItem('booked', JSON.stringify(addedBooked))
